...
public class LazyMapTest<K, V> extends AbstractIterableMapTest<K, V> {
    ...
    public void testMapGet() {
        // Exercise LazyMap.get() in both branches:
        //  (a) missing key -> factory creates value and the method returns that value
        //  (b) existing key -> returns the value already stored in the backing map
        final Map<Object, Number> map = lazyMap(new HashMap<Object, Number>(), oneFactory);
        assertEquals(0, map.size());

        final Number created = map.get("Five");
        assertEquals(1, created);
        assertEquals(1, map.size());

        // Same logical key via a different String instance must return the same cached value
        final Number cached = map.get(new String(new char[] { 'F', 'i', 'v', 'e' }));
